...
class HealthData {
    ...
    ///  using the data types within `HealthData.allHealthDataTypes`
...
//            fatalError("Health data is not available!")
            log("Health data is not available!")
        }

        log("Requesting HealthKit authorization...")
        healthStore.requestAuthorization(toShare: shareTypes, read: readTypes) { (success, error) in
            if let error = error {
                log("requestAuthorization error: \(error.localizedDescription)")
            }

            if success {
                log("HealthKit authorization request was successful!")
            } else {
                log("HealthKit authorization was not successful.")
            }
    ...
    class func fetchStepCountStatistics(with identifier: HKQuantityTypeIdentifier,
                                        completion: @escaping ([StepCountModel]) -> Void) {
...
//            fatalError("*** Unable to create a step count type ***")
            log("*** Unable to create a step count type ***")
            return
        }
    ...
    class func updateAnchor(_ newAnchor: HKQueryAnchor?, from query: HKAnchoredObjectQuery) {
        if let sampleType = query.objectType as? HKSampleType {
            ...
            if let identifier = query.objectType?.identifier {
                let errorMessage = "anchoredObjectQueryDidUpdate error: Did not save anchor for" +
                "\(identifier) – Not an HKSampleType."
                log(errorMessage)
            } else {

                let errorMessage = "anchoredObjectQueryDidUpdate error:" +
                " query doesn't not have non-nil objectType."
                log(errorMessage)
            }
        }
    }
